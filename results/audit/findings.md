...
재현 환경: `results/snapshots/repro_run3/` + `results/exp10_room_gen/room_gen.py` (`.venv/Scripts/python.exe`). ward linkage는 결정적이라 동일 입력에서 동일 출력 확인. LLM 호출은 비용 발생을 피해 생략 (지시대로).
