...
| exp5 (방 병합, type 분류) | `results/exp5/exp5_lib.py`, `exp5_embed.py`, `exp5_llm.py`, `exp5_llm_v2.py`, `type_select_test.py` | `results/exp5/stage1_payloads.json`, `stage2_emb_K{5,8,10}.json`, `stage2_emb_K8_alt_average.json`, `stage2_llm_v2_K5_run{1,2,3}.json`, `embed_silhouette_summary.json`, `embed_reliability.json`, `llm_reliability.json`, `llm_v2_reliability.json`, `llm_v2_raw/run{1,2,3}.txt`, `entity_breakdown.html`, `entity_breakdown_v2.html`, `dedup_candidates.md`, `type_select_test.md`. 리포트: `results/reports/05_exp5_data_contract.md` (spec, 결과 수치는 아님) | (스냅샷 안 만듦. `repro_run3` 입력 재사용.) |
| exp6 (임베딩 직접 클러스터 vs community 병합) | `results/exp6_room_probe/probe.py` | `results/exp6_room_probe/report.md` (직접 ward K=10 크기 분포, 클러스터별 멤버표, orphan 31개 매핑, community-merged 비교) | (스냅샷 안 만듦. `repro_run3` 입력) |
| exp7 (방 위 LLM 레이어, rubric, 3런 안정성) | `results/exp7/probe.py` | `results/exp7/report.md`, `results/exp7/raw/run{1,2,3}/` (LLM 원응답) | (스냅샷 안 만듦. `repro_run3` 입력) |
| exp8 (목차/섹션 feasibility) | `results/exp8_toc_feasibility/probe.py` | `results/exp8_toc_feasibility/report.md` (46섹션, text_unit 매핑, 엔티티 다중섹션 분포) | (스냅샷 안 만듦. `repro_run3` 입력 + 원본 텍스트) |
| exp9 (semantic vs pagesplit rechunk) | `results/exp9_rechunk/build_inputs.py`, `run_full.py`, `run_verify.py`, `eval_run.py` | `results/exp9_rechunk/comparison.md`, `eval_semantic_run1.json`, `eval_pagesplit_run1.json`. `logs/exp9_*.log`, `logs/{pagesplit_run1,semantic_run1}/indexing-engine.log`. proj 설정: `proj_pagesplit/settings.yaml`, `proj_semantic/settings.yaml` | `results/snapshots/semantic_run1/`, `results/snapshots/pagesplit_run1/` |
...
| degree 상위 (조선 41 / 사림 17 / 정조 17 / 임진왜란 16 / 영조 12) | `results/snapshots/repro_run3/entities.parquet` 그리고 `results/exp6_room_probe/report.md`의 cluster 내 degree 표 | 실물 있음 (parquet 컬럼으로 재계산 가능) |
| orphan 31개에 측우기·자격루·앙부일구·혼천의·인지의·금속활자 등이 들어 있음 | `results/exp6_room_probe/report.md` 의 orphan 매핑 표 | 실물 있음 |
...
| exp5 type-keep 142 / type-demote 104 / unknown 111 (전체의 31%) | `results/exp5/` 안 (`exp5_lib.py`, `type_select_test.{py,md}`, `entity_breakdown_v2.html`) | 실물 있음 (HTML 분류 결과). 정확한 수치 매칭은 다음 단계. |
| exp6 직접 클러스터 [51,50,48,45,44,34,24,23,20,18] vs community-merged [160,45,35,34,18,10,8,7,5,4] | `results/exp6_room_probe/report.md` 표 + `results/exp5/stage2_emb_K10.json` | 실물 있음 |
| exp6 임진왜란 7/8 동일 클러스터, 세종 과학 4/9 모드 클러스터 | `results/exp6_room_probe/report.md` 앵커 표 | 실물 있음 |
| exp7 클러스터 2 jaccard 0.98, 클러스터 3 0.17, 클러스터 9 0.00 | `results/exp7/report.md` 3런 안정성 표. raw `results/exp7/raw/run{1,2,3}/` | 실물 있음 |
| exp7 클러스터 6만 grab-bag, 나머지 9개 coherent | `results/exp7/report.md` coherence 표 | 실물 있음 |
| exp7 run3에서 이순신·임진왜란·거북선·권율·김시민·곽재우 missing | `results/exp7/report.md` SHOULD-SHOW O/X 표. raw `results/exp7/raw/run3/` | 실물 있음 |
| exp8 357개 엔티티 100%가 2+ 섹션 걸침, 평균 5.12 | `results/exp8_toc_feasibility/report.md` | 실물 있음 |
| exp8 측우기 [13,14,15,16], 이순신 [21,22,23,24,25] 등 | `results/exp8_toc_feasibility/report.md` 스팟체크 표 | 실물 있음 |
| exp9 semantic 1038 / pagesplit 755 entities | `results/exp9_rechunk/eval_{semantic,pagesplit}_run1.json` + `results/snapshots/{semantic,pagesplit}_run1/entities.parquet` | 실물 있음 |
| exp9 orphan 10.5% / 17.1%, max cluster 243 / 135 | `results/exp9_rechunk/eval_{semantic,pagesplit}_run1.json` + `comparison.md` | 실물 있음 |
...
- **근거 코드**: `results/exp6_room_probe/probe.py` (직접 ward) + `results/exp5/exp5_embed.py` (community 병합 baseline)
- **근거 결과**: `results/exp6_room_probe/report.md` 의 "2. 크기 분포 비교" 표 (`[51,50,48,45,44,34,24,23,20,18]` vs `[160,45,35,34,18,10,8,7,5,4]`). `results/exp5/stage2_emb_K10.json` 이 community 병합 원본.
...
- **두 가지 다른 LLM 병합이 있어 헷갈리기 쉬워서 분리**:
  - exp5 v1 "partition" LLM 병합 (40방을 K개 그룹으로 LLM이 분배): `results/exp5/llm_reliability.json` 에서 4회 전부 실패 기록. stage2 JSON 미생성. → "임베딩이 압도적으로 나음"의 강한 근거.
...
- **근거 코드**: `results/exp5/exp5_llm.py`, `exp5_llm_v2.py`, `results/exp10_room_gen/room_gen.py` 의 `_merge_llm` 함수.
