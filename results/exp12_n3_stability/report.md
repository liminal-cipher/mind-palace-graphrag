...
- 앵커: `results/exp10_room_gen/anchors_korean_history.json` (한국사). should_show 14 / should_demote 8.
...
이 디렉토리는 클러스터별 결과·앵커만 본다. 방 단위 keep-set이 패스 간 얼마나 일치하는지, 다수결이 몇 개 엔티티를 정리했는지는 confirmed-pipeline runner (`results/pipeline/`)가 측정한다. 참조: [`results/pipeline/report.md`](../pipeline/report.md). 거기에 다음이 있다:
...
- Stage B: 클러스터 × n_runs × K 조합. K=10×3 = 30 호출, K=5×3 = 15 호출. 합 45 호출. 토큰·달러는 confirmed-pipeline 보고서(`results/pipeline/report.md`)에 별도 기록.
