...
baseline(`<REPO>/prompts/`) 충돌 확인: 메인 `settings.yaml`이 `prompts/extract_graph.txt` 사용, exp9 `proj_pagesplit`도 `../prompts/extract_graph.txt`로 같은 폴더 사용. **exp17 로컬 폴더 `results/exp17_generalization/prompts/`는 baseline과 겹치지 않음.** prompt-tune이 baseline을 덮어쓸 위험 0.
...
- `results/exp17_generalization/clean_corpus.py` (결정적 정리 스크립트)
- `results/exp17_generalization/cleanup_report.json` (정리 통계)
- `results/exp17_generalization/settings.yaml` (exp17 설정)
- `results/exp17_generalization/prompts/extract_graph.txt` (생성, 라벨 점검 필요)
- `results/exp17_generalization/prompts/summarize_descriptions.txt` (생성)
- `results/exp17_generalization/prompts/community_report_graph.txt` (생성, 안 씀)
- `results/exp17_generalization/PHASE_A_CHECKPOINT.md` (이 보고서)
