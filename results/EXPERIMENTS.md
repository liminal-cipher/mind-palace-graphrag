...
**한 일**: 같은 한국사 자료(20,921자, 교과서 정제본)에 baseline 인덱싱을 돌리고(385 entities, 31 level 0 커뮤니티, 16분, 1.02달러), 같은 입력 위에서 `max_cluster_size`와 `use_lcc`(고립된 그래프 섬을 버릴지) 같은 하이퍼파라미터를 바꿔 가며 N=3으로 자연 편차를 측정. 따로 repro_run3 스냅샷(357 엔티티, 40방, orphan 31개)에서 degree 분포와 시간 프로파일을 봤다.

**결과**
- 자연 편차: 같은 설정으로 새로 추출하면 level 0 방 개수가 30, 32, 40 (±10). LLM 비결정성 때문. → 매번 새로 추출하면 비교가 안 되니 스냅샷 기반 작업으로 전환.
- degree 상위가 외울 핵심과 안 맞음: degree 1위 `조선`(41), 2위 `사림`(17), 3위 `양반`(17) 같은 일반어가 상위를 차지. 외울 만한 고유명사·구체적 사물은 더 아래에 깔림.
- orphan 31개(8.7%): 자료엔 등장하지만 관계가 안 잡혀 고립된 엔티티. 그중에 측우기·자격루·앙부일구·혼천의 같은 과학기기, 의병 인물 7명, 주요 사료 등 외울 핵심이 다수.
...
- orphan 31개 중 과학기기 4종(측우기·자격루·앙부일구·혼천의)이 keep으로 살아남고, 사료 7종(고려사·동국통감 등)도 keep 쪽에 들어옴.
...
- 주제 응집 앵커 체크:
  ...
  - 세종 과학 그룹: 도구 4종이 한 클러스터에 모이고 세종·훈민정음도 인접 클러스터.
...
- 방 이름 안정성: 10개 중 6개는 3런이 같은 이름 또는 1글자 차이(`조선 초기 군주와 군사`, `조선 법전과 문헌`, `조선 행정과 자치 제도` 등). 클러스터 0(임진왜란)·4(조선 정치와 의병)·5(조선-일본 교역)는 똑같은 이름 유지.
- keep 선별 안정성(3런 jaccard 평균): 클러스터 2(법전·문헌) 0.98, 1(군주·군사) 0.87, 8(행정) 0.83, 6(인물·지리·제도) 0.77 → 일관성 높음. 클러스터 3(교육·사림 붕당)·9(북방 군사) 등은 0.17~0.33으로 흔들림이 있음.
- coherence 플래그: 10개 중 9개는 3런 모두 `coherent`로 평가. 클러스터 6 한 곳만 `grab-bag`(잡동사니)으로 매번 같게 평가 → LLM이 클러스터 품질도 일관되게 본다.
- should-show 앵커(외울 핵심): 측우기·자격루·앙부일구·혼천의·이순신·임진왜란·거북선·세종·정약용 등 대부분 3런 모두 keep. should-demote 앵커(`조선`·`백성`·`성리학`·`경상도`)도 3런 모두 demote로 일관.
- 비용: 3런 × (rubric 1번 + 클러스터 10번) = 33회 호출, prompt 88k + completion 23k 토큰.

**그래서**: LLM 선별기는 클러스터가 좀 지저분해도(grab-bag) 안의 keep/demote 판단은 흔들지 않음. 방 이름은 같은 의미 다른 단어 정도로만 흔들림. 임베딩 클러스터 + LLM 선별 조합을 후속 방 병합 파이프라인의 기본 단위로 채택할 수 있다.
...
**결과**
- text_unit이 너무 큼: 청크 한 개가 평균 5.12개 섹션에 걸침. 12개 청크 모두 4~8개 섹션을 한꺼번에 덮음.
