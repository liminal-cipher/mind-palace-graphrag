...
## 다음 할 일 (방향)
1. LLM 병합 v2 (assignment 방식): "K개 그룹 만들어라"가 아니라 "community 0~39 각각에 그룹 라벨을 붙여라".
   입력을 훑으며 라벨만 다니까 누락·중복이 구조적으로 거의 안 생김. LLM에 공정한 형태로 재시도.
2. 하이브리드: 임베딩이 묶고 LLM이 검수(필요하면 교정). LLM 검수는 주제를 안 가려서 다른 자료로 확장 가능.
3. 3-way 비교: 임베딩 / LLM(v2) / 하이브리드 × {완전성, 재현성, 품질, 크기분포, 시간, 비용}.
4. 재현 패키지: README, requirements.txt, .env.example, run_all.py.
...
## 환경
- Azure OpenAI: gpt-4.1-mini(병합) + text-embedding-3-small(추출). 엔드포인트는 코드에, 키는 `.env`(커밋 금지).
...
- 리포트(results/): Step1_스냅샷비교 / Step2_재현성 / Step3_종합 / 실험2 / 실험4_lcc / baseline / exp5_data_contract.
