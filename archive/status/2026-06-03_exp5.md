...
- 리포트는 `archive/reports/` 아래 `NN_slug.md` 형식. frontmatter 규약: `archive/reports/REPORT_TEMPLATE.md`. 원본에서 현재 파일로의 매핑: `archive/reports/INDEX.md`.
...
- 두 번 돌려 완전히 동일(run_a == run_b). 결과: `archive/exp05_stage2_merge/stage2_emb_K{5,8,10}.json`.
...
- 원인: LLM은 40개 ID를 빠짐없이 분배하는 "부기" 작업에 약함. 로그: `archive/exp05_stage2_merge/llm_reliability.json`.
...
- exp5: 방 병합 실험 (현재). 코드는 `archive/exp05_stage2_merge/` 아래 `exp5_lib.py` / `exp5_embed.py` / `exp5_llm.py` / `exp5_llm_v2.py` / `type_select_test.py`.
- 리포트(`archive/reports/`): `00_baseline`, `01_max15`, `02_snap_max10`, `02_snap_max20`, `03_repro_step{1_snapshot, 2_variance, 3_summary}`, `04_use_lcc`, `05_exp5_data_contract`. 규약과 매핑은 `REPORT_TEMPLATE.md`, `INDEX.md`.
