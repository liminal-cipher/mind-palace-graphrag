...
- TOC 배정: `archive/exp15_toc_chapters/entity_chapter_assignments.csv` 의 `dominant_chapter_B` 컬럼 (B 파티션 6챕터)
- 챕터 정의: `archive/exp15_toc_chapters/chapter_definition.json` `partition_B`
- 앵커: `archive/exp10_room_gen/anchors_korean_history.json` (should_show 14 + should_demote 8, alias 적용)
...
## 결과 요약 (archive/exp16_room_compare/metrics.json)
...
- 데이터: `archive/exp16_room_compare/blind_compare.json`. `sets.set1`, `sets.set2` 각각 6방, 방마다 엔티티 이름 목록. 어느 쪽이 TOC인지·챕터 이름은 이 파일에 없다.
- 키: `archive/exp16_room_compare/blind_key.json`. 중립 라벨(set1_room1..6, set2_room1..6) → 실제 방식·방 ID·(TOC만) 챕터 정보. 사람 평가가 끝난 뒤 이 키로 공개.
...
- `archive/exp16_room_compare/build.py`: 빌드 스크립트 (no-LLM, 결정적).
- `archive/exp16_room_compare/toc_rooms.json`: TOC 방 6개, entity→방 배정.
- `archive/exp16_room_compare/graph_rooms.json`: 그래프 ward 방 6개, entity→방 배정.
- `archive/exp16_room_compare/metrics.json`: 커버리지·동거·재현성 머신리더블.
- `archive/exp16_room_compare/blind_compare.json`: 중립 라벨 블라인드 데이터.
- `archive/exp16_room_compare/blind_key.json`: 중립 라벨 → 실제 방식 매핑.
